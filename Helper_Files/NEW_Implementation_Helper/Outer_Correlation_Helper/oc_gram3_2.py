...
# Run for category: gram3-comparative
category_name = "gram3-comparative"
questions = sections[category_name]
half_index = len(questions) // 2
questions_part1 = questions[:half_index]
questions_part2 = questions[half_index:]
print(f"Processing category: {category_name} ({len(questions)} questions)")

# ADD THIS SECTION - Pre-compute correlation matrices
print("Pre-computing all correlation matrices...")
corr_matrices = {}
for freq in freq_subs:
    for rc, q in zip(rc_vals, quantiles):
        key = (freq, rc, q)
        print(f"  Computing matrix for freq={freq}, rcond={rc}, quantile={q}")
        Cw_inv, center_vec = build_correlation_and_center(model, freq, rc, word2count)
        corr_matrices[key] = (Cw_inv, center_vec)
print(f"Successfully pre-computed {len(corr_matrices)} correlation matrices")

# REPLACE the run_all_experiments function with this custom implementation
# Select the right part for each file
part_idx = 2  # For gram3-comparative-2.py
chunk_questions = questions_part2  # Use questions_part1 or questions_part2
print(f"Processing part {part_idx} of 2...")
...
for freq in freq_subs:
    for rc, q in zip(rc_vals, quantiles):
        ...
        key = (freq, rc, q)
        Cw_inv, center_vec = corr_matrices[key]
...
final_df = pd.concat(all_results, ignore_index=True)
out_csv = f"gram3_comparative_results-{part_idx}.csv"
...
print(f"Results for '{category_name}' part {part_idx} saved to {file_path}!")

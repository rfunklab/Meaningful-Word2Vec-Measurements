...
# #### Helper Function to Build Outer Correlation Matrices

# In[ ]:

def build_outer_correlation(
    model: "KeyedVectors",
    freq_subset: int,
    rcond: float,
    ...
    """
    Returns a 300 × 300 (D × D) outer‑correlation matrix of the chosen
    subset of the most‑frequent words.
    
    The (i,j) entry is
           Corr(i,j) =  Σ_k (x_{k,i} − μ_i)(x_{k,j} − μ_j)
                       ------------------------------------
                           (N‑1) σ_i  σ_j
    where k indexes the freq_subset words, μ is the per‑dimension mean
    and σ the per‑dimension sample standard‑deviation.
    
    Args:
        model: gensim.models.KeyedVectors
            Model that supplies vectors and token counts.
        freq_subset: int
            Number N of most‑frequent words (with count ≥ 5) used to build the
            matrix.
        word2count: Dict[str, int]
            Dictionary mapping words to their frequency counts.
        rcond: float, default 0.0
            **Optional** ridge‑like regularisation: any singular values smaller
            than `rcond * max(s)` are set to that floor before normalisation.
            (Set to 0.0 for the exact correlation matrix.)
    
    Returns:
        np.ndarray
            (D, D) correlation matrix – for GoogleNews this is 300 × 300.
    """
    ...
    top_words = [word for word, * in sorted*vocab[:freq_subset]]
    V = np.asarray([model[w] for w in top_words], dtype=np.float64)
    N, D = V.shape
    ...
    # Prevent divide‑by‑zero for rarely‑used dimensions
    std[std == 0.0] = 1.0
    Xcs = Xc / std
    # Outer correlation matrix
    Ccorr = np.einsum('ni,nj->ij', Xcs, Xcs) / (N - 1)
    # SVD regularization
    U, s, _ = np.linalg.svd(Ccorr, hermitian=True)
    cutoff = rcond * s.max()
    s[s < cutoff] = cutoff
    Ccorr = (U * s) @ U.T
    return Ccorr

def build_correlation_and_center(
    model: "KeyedVectors",
    freq_subset: int,
    rcond: float,
    word2count: Dict[str, int]
    ...
    """
    Build the outer correlation matrix and compute the center of mass using the top `freq_subset`
    frequent words from the model. This function combines the correlation matrix computation
    with center of mass calculation for convenience.
    ...
    Args:
        ...
        rcond: A float specifying the cutoff for small singular values in regularization.
        word2count: Dictionary mapping words to their frequency counts.
        
    Returns:
        tuple: (Ccorr, cm), where
            Ccorr is the outer correlation matrix,
    ...
    # Build correlation matrix with correct parameter order
    Ccorr = build_outer_correlation(model, freq_subset, rcond, word2count)
    # Center of mass
...
def evaluate_analogies(
    ...
    Args:
        ...
        Cw_inv (np.ndarray, optional): Outer correlation matrix (for correlation-based measures).
        center_vec (np.ndarray, optional): Center vector for correlation shifted cosine.
    ...
    for (w1, w2, w3, w4, category) in questions:
        ...
        # Sort candidates
        sorted_candidates = [x for *, x in sorted(zip(best*scores, best_candidates), reverse=True)]
...
def run_all_experiments(
    ...
    build_corr_center_fn: Callable[[KeyedVectors, int, float, Dict[str, int]], Tuple[np.ndarray, np.ndarray]],
    ...
    Args:
        ...
        freq_subsets (list of int): 
            ...
            (with count ≥ 5) to consider when computing the outer correlation matrix.
            Typically `[30000]`.
        rcond_values (list of float): 
            A list of cutoff values for small singular values when computing the 
            outer correlation matrix using SVD regularization. 
        ...
        build_correlation_and_center (function): 
            A function that computes the outer correlation matrix 
            using a subset of frequent words. Expected signature:
            ```python
            build_correlation_and_center(model, freq_subset, rcond, word2count) -> (Ccorr, center_vec)
            ```
            where:
            - `Ccorr`: Outer correlation matrix.
            - `center_vec`: Center-of-mass vector (used in correlation shifted cosine).
    ...
    Returns:
        pd.DataFrame: 
            ...
            - `"rcond"`: Regularization value used in outer correlation matrix
    ...
    Raises:
        ...
        RuntimeError: If outer correlation computation fails.
    """
    all_results = []
    
    for freq in freq_subsets:
        for rc, q in zip(rcond_values, quantiles):
            Cw_inv, center_vec = build_corr_center_fn(model, freq, rc, word2count)
...
# #### Compute rcond values from eigenvalues for Correlation matrix

# In[ ]:
def determine_optimal_corr_rcond(
    model: "KeyedVectors",
    ...
    """
    Determines optimal rcond value for correlation matrix by selecting the eigenvalue at the given quantile.
    ...
    Returns:
        float: Optimal rcond for correlation matrix.
    """
    ...
    # Compute correlation matrix (without regularization) to get eigenvalues
    N, D = subset_vectors.shape
    mu = subset_vectors.mean(axis=0, keepdims=True)
    Xc = subset_vectors - mu
    std = Xc.std(axis=0, ddof=1)
    std[std == 0.0] = 1.0
    Xcs = Xc / std
    Ccorr = np.einsum('ni,nj->ij', Xcs, Xcs) / (N - 1)
    ...
    # Get eigenvalues
    corr_eigvals = np.linalg.eigvalsh(Ccorr)[::-1]
    corr_quantile_value = np.quantile(corr_eigvals, quantile)
    return corr_quantile_value / corr_eigvals[0]

# In[ ]:
corr_rcond_values = {
    q: determine_optimal_corr_rcond(model, freq_subs, quantile=q, word2count=word2count)
    for q in quantiles
}

# In[ ]:
rc_vals = list(corr_rcond_values.values())

# #### Running the Code in Chunks for Optimization

# In[ ]:

# Run for section: capital-common-countries
...
results_df = run_all_experiments(
    ...
    build_corr_center_fn=build_correlation_and_center,
    evaluate_fn=evaluate_analogies,
    word2count=word2count,
    out_csv="capital_common_countries_results.csv",
    out_dir="new_outer_correlation_results_per_section",
...
results_df = run_all_experiments(
    ...
    evaluate_fn=evaluate_analogies,
    word2count=word2count,
    out_csv="capital_world_results.csv",
    out_dir="new_outer_correlation_results_per_section",
    top_k_values=[1, 3, 5, 10]
)
print(f"Results for '{section_name}' saved!")
...
# Run for section: city-in-state
section_name = "city-in-state"
questions = sections[section_name]
print(f"Processing section: {section_name} ({len(questions)} questions)")
results_df = run_all_experiments(
    model=model,
    ...
    evaluate_fn=evaluate_analogies,
    word2count=word2count,
    out_csv="city_in_state_results.csv",
    out_dir="new_outer_correlation_results_per_section",
    top_k_values=[1, 3, 5, 10]
)
print(f"Results for '{section_name}' saved!")
...
# Run for section: gram1-adjective-to-adverb
...
results_df = run_all_experiments(
    ...
    evaluate_fn=evaluate_analogies,
    word2count=word2count,
    out_csv="gram1_adjective_to_adverb_results.csv",
    out_dir="new_outer_correlation_results_per_section",
    top_k_values=[1, 3, 5, 10]
)
print(f"Results for '{section_name}' saved!")
...
# Run for section: gram2-opposite
section_name = "gram2-opposite"
questions = sections[section_name]
print(f"Processing section: {section_name} ({len(questions)} questions)")
results_df = run_all_experiments(
    model=model,
    ...
    evaluate_fn=evaluate_analogies,
    word2count=word2count,
    out_csv="gram2_opposite_results.csv",
    out_dir="new_outer_correlation_results_per_section",
    top_k_values=[1, 3, 5, 10]
)
print(f"Results for '{section_name}' saved!")
...
# Run for section: gram3-comparative
section_name = "gram3-comparative"
questions = sections[section_name]
print(f"Processing section: {section_name} ({len(questions)} questions)")
results_df = run_all_experiments(
    model=model,
    ...
    evaluate_fn=evaluate_analogies,
    word2count=word2count,
    out_csv="gram3_comparative_results.csv",
    out_dir="new_outer_correlation_results_per_section",
    top_k_values=[1, 3, 5, 10]
)
print(f"Results for '{section_name}' saved!")
...
# Run for section: gram4-superlative
section_name = "gram4-superlative"
questions = sections[section_name]
print(f"Processing section: {section_name} ({len(questions)} questions)")
results_df = run_all_experiments(
    model=model,
    ...
    evaluate_fn=evaluate_analogies,
    word2count=word2count,
    out_csv="gram4_superlative_results.csv",
    out_dir="new_outer_correlation_results_per_section",
    top_k_values=[1, 3, 5, 10]
)
print(f"Results for '{section_name}' saved!")
...
# Run for section: gram5-present-participle
section_name = "gram5-present-participle"
questions = sections[section_name]
print(f"Processing section: {section_name} ({len(questions)} questions)")
results_df = run_all_experiments(
    model=model,
    ...
    evaluate_fn=evaluate_analogies,
    word2count=word2count,
    out_csv="gram5_present_participle_results.csv",
    out_dir="new_outer_correlation_results_per_section",
    top_k_values=[1, 3, 5, 10]
)
print(f"Results for '{section_name}' saved!")
...
results_df = run_all_experiments(
    ...
    evaluate_fn=evaluate_analogies,
    word2count=word2count,
    out_csv="gram7_past_tense_results.csv",
    out_dir="new_outer_correlation_results_per_section",
    top_k_values=[1, 3, 5, 10]
)
print(f"Results for '{section_name}' saved!")
...
# Run for section: gram8-plural
section_name = "gram8-plural"
questions = sections[section_name]
print(f"Processing section: {section_name} ({len(questions)} questions)")
results_df = run_all_experiments(
    model=model,
    ...
    evaluate_fn=evaluate_analogies,
    word2count=word2count,
    out_csv="gram8_plural_results.csv",
    out_dir="new_outer_correlation_results_per_section",
    top_k_values=[1, 3, 5, 10]
)
print(f"Results for '{section_name}' saved!")
